...
import { Users, Plus, Search, ShieldAlert, CheckCircle2, Lock, MoreVertical, Building2, Trash2 } from 'lucide-react';
...
export const MemberManager: React.FC<MemberManagerProps> = ({ currentUser }) => {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ isOpen: boolean; member: UserProfile | null; loading: boolean }>({
  ...
  const handleDeleteMember = async () => {
    if (!deleteConfirm.member?.id) return;
    setDeleteConfirm(prev => ({ ...prev, loading: true }));
    try {
      await deleteAccount(deleteConfirm.member.id);
      setDeleteConfirm({ isOpen: false, member: null, loading: false });
      await fetchMembers();
    } catch (error: any) {
                        ...
                        <div className="relative group inline-block text-left">
                          ...
                          <div className="absolute right-0 w-36 mt-1 origin-top-right bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                            <div className="py-1">
                              ...
                              <button
                                onClick={() => setDeleteConfirm({ isOpen: true, member, loading: false })}
                                className="w-full text-left px-4 py-2 text-sm text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2"
                              >
        ...
        <div className="fixed inset-0 bg-slate-900/75 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl max-w-sm w-full overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700">
              <h3 className="font-semibold text-lg text-rose-600 flex items-center gap-2">
                <Trash2 className="w-5 h-5" /> Xóa tài khoản
              </h3>
            </div>
            <div className="p-6 space-y-3 text-sm">
              <p className="text-slate-700 dark:text-slate-300">
                Bạn có chắc muốn xóa tài khoản thành viên này?
              </p>
              <div className="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-3 space-y-1">
                <div className="font-medium">{deleteConfirm.member.fullName}</div>
                <div className="text-xs text-slate-500">{deleteConfirm.member.email}</div>
              </div>
              <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-3 text-xs text-amber-700 dark:text-amber-400">
                <strong>Lưu ý:</strong> Tài khoản và thông tin đăng nhập sẽ bị xóa vĩnh viễn. Người dùng sẽ có thể đăng ký lại bằng email này.
              </div>
            </div>
            <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-700 flex justify-end gap-3">
              <button
                onClick={() => setDeleteConfirm({ isOpen: false, member: null, loading: false })}
                disabled={deleteConfirm.loading}
                className="px-4 py-2 text-sm text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-md"
              >
                Hủy
              </button>
              <button
